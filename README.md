# 🌐 Web Development Learning & Reference Repository
...
## 🏆 Priority-Wise Learning Roadmap

To build a strong foundation as a web developer, follow these modules in order:
...
| **2** | [**`JavaScript/`**](./JavaScript/) | **The Core Programming Language:** Complete JavaScript from fundamentals (variables, types, loops) to modern ES6+, DOM manipulation, and asynchronous programming. | • [Topper Notes PDF](./JavaScript/JavaScript_Topper_Handwritten_Notes.pdf)<br>• [Short Notes PDF](./JavaScript/JavaScript_Short_Notes.pdf) |
| **3** | [**`Backend/Terminal/`**](./Backend/Terminal/) | **Server Environment & CLI Navigation:** Command-line fluency, operating system file structures, shell scripting basics, and environment preparation for Node.js. | • [Terminal Notes PDF](./Backend/Terminal/Backend_Terminal_Handwritten_Notes.pdf)<br>• [Interactive HTML](./Backend/Terminal/Backend_Terminal_Topper_Notes.html) |
...
│   ├── 📄 Git_Github_Handwritten_Notes_English.pdf   # English handwritten topper notes (6 pages)
│   ├── 📄 Git_Github_Handwritten_Notes_Hinglish.pdf  # Hinglish handwritten topper notes (5 pages)
│   ├── 📄 Git_Github_Topper_Notes_English.html       # Printable styled HTML notes (English)
│   ├── 📄 Git_Github_Topper_Notes_Hinglish.html      # Printable styled HTML notes (Hinglish)
...
- **VCS Concepts:** Distributed vs Centralized, 3 Stages of Git (Working, Staging, Local Repo).
- **Core Commands:** `init`, `clone`, `status`, `add`, `commit`, `push`, `pull`, `log`.
...
- **Navigation & Paths:** `pwd`, `ls -la`, `cd`, Absolute vs Relative paths (`~`, `/`, `..`).
- **File Management:** `touch`, `mkdir -p`, safe deletion rules (`rm`, `rm -rf`).

### 🟢 [4. Backend / Node.js](./Backend/NodeJS/)
- **Runtime Environment:** V8 engine, libuv, event loop, non-blocking I/O.
- **Process Management:** `process.argv`, `process.cwd()`, `process.exit()`.
- **Module Architecture:** CommonJS (`module.exports`, `require`) vs ES6 Modules (`import`, `export`).
...
```bash
git clone https://github.com/S1h2i3v4a/Web_Development.git
cd Web_Development
```
